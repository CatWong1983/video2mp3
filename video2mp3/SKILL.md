...
- **"验证码中间页" = 抖音风控**（同一 profile/IP 高频抓取触发滑块）。恢复方式：有头窗口里手动过一次滑块，脚本检测到"无 video 元素 + 标题像墙页"时会在**用户闲置 30 秒后**自动重新导航到目标页（正在扫码/拖滑块时不打断），过完即继续；仍被挡可删除 `~/.video2mp3/browser-profile` 重置，或 CDP 接管用户日常 Chrome。避免短时间对同一 profile 高频抓取
