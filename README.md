...
- **播放队列保护**：QQ 音乐会用打开的文件替换整个播放队列，所以默认只激活窗口不清队列；网易云实测不清队列，直接播放
...
|---|---|---|
| 打开 mp3 自动播放 | ✅ | ✅ |
| 替换当前播放队列 | ⚠️ 会清（默认只激活窗口保护队列） | ✅ 不清（默认直接播放） |
| 自动入本地曲库 | ✅（open 播放过的文件自动导入） | ❌ 需手动添加文件夹 |

### 曲库入库（选做）

不入库也不影响"转换完立刻听"，文件始终在输出目录。只有想在 app「本地音乐」里长期管理才需要：

- **QQ 音乐**（建议做）：「本地歌曲」→「手动添加 / 添加本地歌曲文件夹」→ 选 `~/Music/video2mp3`
- **网易云**（纯临时听可跳过）：「本地音乐」里手动添加 `~/Music/video2mp3` 文件夹

## 环境变量

| 变量 | 取值 | 说明 |
|---|---|---|
| `VIDEO2MP3_ACTION` | `open`（默认）/ `play` / `none` | open：QQ音乐只激活窗口不清队列，网易云直接播放；play：强制自动播放（QQ音乐会清队列）；none：只保存文件 |
